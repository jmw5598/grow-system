...
export class TemperatureHumidityAction {
  ...
  start(): void {
    this._interval = setInterval(() => {
      DHTSensor.read(22, this._component.pin, (error: any, temperature: number, humidity: number) => {
        if (!error) {
  ...
  _notify(temperature: number, humidity: number): void {
    ...
    if (
      temperature > this._component.preferences.threshold.max ||
      temperature < this._component.preferences.threshold.min
    ) {
