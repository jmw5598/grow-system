...
export class SystemNodeComponentService implements IMessageService {
  ...
  constructor(channel: IPubSubChannel) {
    ...
    this._applicationContext.getItem('actions').subscribe((actions: any) => (this._actions = actions));
  ...
  public processMessage(message: MqttMessage): void {
    switch (message.topic) {
      case 'create':
        return this._createComponent(message);
      case 'update':
        return this._updateComponent(message);
      case 'delete':
        return this._deleteComponent(message);
      default:
  ...
  private _createComponent(message: MqttMessage): void {
    ...
    this._actions.push(ActionFactory.create(component.type.model, this._config.node, component));
